...
import { 
  ...
  Maximize, Minimize, Eye, Crosshair, Filter, ShieldAlert
} from "lucide-react";
...
const MapaEspacial = ({ loteActivo, proyectoActivo, baseDeDatosLotes, isAdmin, onLoteSeleccionado }) => {
  ...
  useEffect(() => {
    ...
    const cargarPlano = async () => {
      try {
        ...
        // Algoritmo para calcular Centroides de UV
        ...
        // Mostrar etiqueta gigante solo si hay más de 1 UV en el proyecto
        if (uvMap.size > 1) {
  ...
  // 2. MATRIZ DE COLORES Y ESTADOS DESDE EXCEL
  const { verdes, rojos, azules, opacityVerde, opacityRojo, opacityAzul } = useMemo(() => {
    ...
    lotesFiltrados.forEach(l => {
      ...
      else v.push(numLote); // Disponibles o Vacíos
    });

    let oV = 1, oR = 1, oA = 1;

    // Filtros visuales exclusivos para el Director (Los asesores ven todo siempre)
    if (isAdmin) {
  ...
  // 4. INTERACCIÓN Y BLOQUEO DE SEGURIDAD
  const handleMapClick = (event) => {
    ...
    // CORTAFUEGOS: Asesores no pueden cotizar lo vendido o bloqueado
    if (loteDB && !isAdmin) {
       const estadoLote = String(loteDB.estado).toUpperCase();
       if (estadoLote === "VENDIDO" || estadoLote === "BLOQUEADO" || estadoLote === "RESERVADO") {
          onLoteSeleccionado({ isError: true, message: `⚠️ Lote ${estadoLote}. Solo el Director puede procesarlo.` });
          return;
       }
    }
    ...
    if (loteDB) {
      ...
      // Si el lote no está en Excel, asume Libre para no trabar la venta
      onLoteSeleccionado({ 
        isError: false, 
      ...
      <div className="flex-1 relative w-full h-full bg-[#020617] min-h-[300px]">
        ...
        <div className="absolute inset-0 z-10 w-full h-full">
          <Map
            ...
            maxZoom={20} 
            interactiveLayerIds={['lotes-fill']} // Eventos de click activados
...
export default function App() {
  ...
  const calcularLimitesMaximos = () => { return { maxCreditoPct: 0, maxContadoPct: 0, maxDescM2: 1, maxContadoM2: 2, maxBonoInicial: 500 }; };
    ...
    <div className="min-h-screen bg-[#020617] relative font-['Plus_Jakarta_Sans'] text-slate-300 overflow-x-hidden selection:bg-cyan-500/30 selection:text-cyan-200 pb-20 w-full max-w-[100vw]">
      ...
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] bg-cyan-950/90 text-cyan-50 px-6 py-3 rounded-full shadow-[0_10px_30px_rgba(6,182,212,0.3)] flex items-center gap-3 font-bold text-sm tracking-wide animate-toast border border-cyan-500/50 backdrop-blur-md w-max">
           <AlertCircle className="w-5 h-5 text-cyan-400" /> {toast}
        </div>
      )}
