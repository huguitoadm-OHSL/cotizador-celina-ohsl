  ...
  const dataVisible = useMemo(() => {
    ...
    return {
      ...
      features: geoData.features.filter(f => 
        f.properties.estado === 'LIBRE' || f.properties.estado === 'DISPONIBLE'
      )
    };
  }, [geoData, isAdmin]);
          ...
          >
            ...
            {hoverInfo && (
              <Popup
                ...
                maxWidth="300px"
              >
                <div className="bg-slate-900/95 backdrop-blur-md border border-cyan-500/50 p-3 rounded-xl shadow-[0_0_20px_rgba(6,182,212,0.3)] text-white font-['Plus_Jakarta_Sans']">
                  <div className="text-[10px] uppercase tracking-widest text-cyan-400 font-black mb-1 flex items-center gap-1">
                    <Info className="w-3 h-3" /> Datos del Terreno
                  </div>
                  <div className="text-xl font-black leading-none mb-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div className="col-span-2">
                      <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider ${
                        hoverInfo.properties.estado === 'LIBRE' || hoverInfo.properties.estado === 'DISPONIBLE' ? 'bg-green-500/20 text-green-400 border border-green-500/30' :
                        hoverInfo.properties.estado === 'VENDIDO' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' :
                        'bg-red-500/20 text-red-400 border border-red-500/30'
                      }`}>
                        {hoverInfo.properties.estado}
                      </span>
                    </div>
                  </div>
                </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      ...
      .mapboxgl-popup-tip { border-top-color: rgba(6, 182, 212, 0.5) !important; }
    `;
  ...
  return (
    <div className="min-h-screen bg-[#020617] relative font-['Plus_Jakarta_Sans'] text-slate-300 overflow-x-hidden selection:bg-cyan-500/30 selection:text-cyan-200 pb-20 w-full max-w-[100vw]">
      ...
      <div className="max-w-[1280px] mx-auto py-8 px-4 sm:px-6 lg:px-12 xl:pl-24 relative z-10 w-full min-w-0">
        ...
        {/* MAPA INTERACTIVO (FUSIÓN JSON-GITHUb) */}
