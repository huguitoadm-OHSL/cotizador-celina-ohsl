...
// ============================================================================
// MATEMÁTICA ESPACIAL: FÓRMULA DE HAVERSINE PARA CÁLCULO DE ARISTAS
// ============================================================================
const calcularDistanciaYAngulo = (coord1, coord2) => {
  ...
  // Normalizar ángulo para que el texto sea legible
  ...
  return { distancia, textAngle, midLng, midLat };
};

// ============================================================================
// NÚCLEO WEBGIS: SINCRONIZACIÓN Y CÁLCULO EN TIEMPO REAL
// ============================================================================
const MapaEspacial = ({ loteActivoFormulario, proyectoActivo, baseDeDatosLotes, isAdmin, onLoteSeleccionado }) => {
  ...
  const [hoverEdges, setHoverEdges] = useState(null); // Estado para las medidas dinámicas
    ...
    fetch(geojsonPath)
      ...
      .then(data => {
        ...
        const featuresEnriquecidos = data.features.map(feature => {
          ...
          let estadoReal = "SIN_DATOS";
          if (loteDB && loteDB.estado) {
             const e = loteDB.estado.toUpperCase();
             if (e === "LIBRE" || e === "DISPONIBLE" || e === "") estadoReal = "DISPONIBLE";
             else if (e === "VENDIDO") estadoReal = "VENDIDO";
             else if (e === "BLOQUEADO" || e === "RESERVADO") estadoReal = "BLOQUEADO";
          }

          // LECTURA ESTRICTA DE SUPERFICIE DESDE LA BD
          const supFinal = loteDB && loteDB.superficie ? loteDB.superficie : "Sin datos";
          ...
          return {
            ...
            properties: {
              ...
              superficieReal: supFinal,
              uvReal: loteDB ? loteDB.uv : (uvLimpia || "-"),
              mznReal: loteDB ? loteDB.mzn : (mznLimpia || "-")
            }
          };
        });

        setGeoData({ ...data, features: featuresEnriquecidos });
  ...
  const dataVisible = useMemo(() => {
    ...
    return {
      ...
      features: geoData.features.filter(f => 
        f.properties.estadoReal === 'DISPONIBLE' || f.properties.estadoReal === 'SIN_DATOS'
      )
    };
  }, [geoData, isAdmin]);
  ...
  const fillLayer = useMemo(() => ({
    ...
    paint: {
      'fill-color': [
        'match', ['get', 'estadoReal'],
        'DISPONIBLE', 'rgba(34, 197, 94, 0.45)', 
        'VENDIDO', 'rgba(59, 130, 246, 0.45)',    
        'BLOQUEADO', 'rgba(239, 68, 68, 0.45)',   
  ...
  const highlightLayer = useMemo(() => ({
    ...
    filter: ['==', ['to-string', ['get', 'loteLimpio']], String(loteActivoFormulario || '')]
  }), [loteActivoFormulario]);

  const textProperty = ['coalesce', ['get', 'name'], ['get', 'Name'], ['get', 'TextString'], ['get', 'Text'], ['get', 'text'], ['get', 'RefName'], ''];
  ...
  // CAPA DE MEDIDAS PERIMETRALES (DINÁMICA)
  ...
  // EVENTOS DEL MOUSE CON INYECCIÓN TRIGONOMÉTRICA
  const handleMouseMove = (e) => {
    ...
    if (features && features.length > 0 && features[0].layer.id === 'lotes-fill') {
      ...
      // Calcular aristas en tiempo real
      if (activeFeature.geometry && (activeFeature.geometry.type === 'Polygon' || activeFeature.geometry.type === 'MultiPolygon')) {
        let coordinates = activeFeature.geometry.type === 'Polygon' 
          ? activeFeature.geometry.coordinates[0] 
          : activeFeature.geometry.coordinates[0][0];
        
        const edgeFeatures = [];
        for (let i = 0; i < coordinates.length - 1; i++) {
          const p1 = coordinates[i];
          const p2 = coordinates[i + 1];
          const { distancia, textAngle, midLng, midLat } = calcularDistanciaYAngulo(p1, p2);
          
          // Filtramos segmentos muy pequeños para evitar superposición de textos
          if (distancia > 3) {
            edgeFeatures.push({
              type: 'Feature',
              geometry: { type: 'Point', coordinates: [midLng, midLat] },
              properties: { 
                label: `${distancia.toFixed(2)}m`, 
                angle: textAngle 
              }
            });
          }
  ...
  const handleMapClick = (e) => {
    ...
    if (features && features.length > 0 && features[0].layer.id === 'lotes-fill') {
      ...
      const loteDB = baseDeDatosLotes.find(l => 
        ...
        (p.uvReal !== "-" ? String(l.uv).replace(/[^0-9]/g, '') === String(p.uvReal) : true)
      );
      ...
      if (loteDB) {
        ...
        onLoteSeleccionado({ 
          isError: false, 
          data: { lote: p.loteLimpio, mzn: p.mznReal, uv: p.uvReal, superficie: p.superficieReal, precio: 0, categoria: 'ESTÁNDAR', estado: p.estadoReal } 
        });
      }
    }
  };
    ...
    <div className={containerClasses}>
      ...
      <div className="flex-1 relative bg-[#060b13] w-full h-full min-h-[400px]">
        ...
        >
          ...
          {dataVisible && (
            <Source id="dynamic-data" type="geojson" data={dataVisible}>
            ...
            >
              <div className="bg-[#0f172a] border border-cyan-500/50 p-4 rounded-2xl shadow-[0_10px_30px_rgba(6,182,212,0.3)] text-white font-['Plus_Jakarta_Sans'] w-48">
                ...
                <div className="flex justify-between items-center text-[10px] text-slate-400 font-bold mb-3">
                  <span>MZN: <span className="text-slate-200">{hoverInfo.properties.mznReal}</span></span>
                  <span>UV: <span className="text-slate-200">{hoverInfo.properties.uvReal}</span></span>
                </div>
                
                <div className="flex justify-between items-center text-[11px] mb-3">
                  <span className="font-bold">Área: <span className="text-amber-400">{hoverInfo.properties.superficieReal} {hoverInfo.properties.superficieReal !== 'Sin datos' && 'm²'}</span></span>
                </div>
                
                <div>
                  <span className={`inline-block px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider ${
                    hoverInfo.properties.estadoReal === 'DISPONIBLE' ? 'bg-green-500 text-[#020617]' :
                    hoverInfo.properties.estadoReal === 'VENDIDO' ? 'bg-blue-500 text-white' :
                    hoverInfo.properties.estadoReal === 'BLOQUEADO' ? 'bg-red-500 text-white' :
                    'bg-slate-700 text-slate-300'
                  }`}>
                    {hoverInfo.properties.estadoReal === 'SIN_DATOS' ? 'No Cotizable' : hoverInfo.properties.estadoReal}
                  </span>
                </div>
              </div>
            </Popup>
...
export default function App() {
  ...
  const [descuentoContado, setDescuentoContado] = useState(2);
